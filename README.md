...
1. **Attention Head Ablation**  
   Targeted ablations to test the causal contribution of individual attention heads, measuring differential accuracy on flat versus parenthesized arithmetic expressions.

2. **Linear Probing**  
   Binary linear probes to test whether parenthesis depth is linearly decodable from attention head outputs and residual stream activations.

3. **Out-of-Distribution Generalization**  
   Evaluation on expressions with nesting depth exceeding the training distribution (depth = 3 vs. training on depth ≤ 2), including ablations to test whether specialized heads remain necessary.
...
Archived documents in docs/archive/ reflect development history and are not required for reproducing results.
...
```bibtex
@software{thomas_step_free_arithmetic_2026,
  title        = {Step-Free Arithmetic Transformers},
  author       = {Thomas, Nishal},
  year         = {2026},
  url          = {https://github.com/Nishal14/Step-free-arithmetic-transformers},
  note         = {Mechanistic interpretability experiments on arithmetic transformers trained with final-answer-only supervision}
}
